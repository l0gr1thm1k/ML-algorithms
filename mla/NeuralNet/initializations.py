...
def uniform(shape, scale=0.5):
    return np.random.uniform(size=shape, low=-shape, high=shape)
...
def get_initializer(name):
    ...
    :param name:  The name of the initialization function as a string.
    :return: the initialization function.
    """
    try:
        return globals()[name]
    except:
        raise ValueError("No such initialization function: %s" % name)
